...
bool CompareEqual(size_t left, size_t right)
{
   return left == right;
}
...
template< typename Key, typename Value > class HashTable
   ...
   public:
      ...
      void Optimize( double loading = 1.5 )
         ...
         newNumberOfBuckets = ( newNumberOfBuckets )? 1:newNumberOfBuckets;
      ...
      HashTable( )
         ...
         numberOfWords = 0;
         buckets = new Bucket<Key, Value> *[ 8192 ];
         for ( int i = 0;  i < numberOfBuckets;  ++i )
            buckets[ i ] = nullptr;
         }
...
//       {
//       Pair *p = hashTable->Find( words[i].c_str( ), 1 );
//       if ( p )
//          ++ (p->value);
//       }
//    return hashTable;
