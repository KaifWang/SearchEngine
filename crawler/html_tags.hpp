...
void copyAndToLower ( const char* sourceStart, const char* sourceEnd, char* target )
   {
   while ( sourceStart != sourceEnd )
      {
      if ( *sourceStart <= 'Z' && *sourceStart >= 'A' )
         *target = 'a' + *sourceStart - 'A';
      else
         *target = *sourceStart;
      sourceStart++;
      target++;
...
DesiredAction LookupPossibleTag ( const char* name, const char* nameEnd )
   ...
   char inputTag[ nameEnd - name + 1 ];
   copyAndToLower ( name, nameEnd, inputTag );

   int lower_idx = 0, upper_idx = NumberOfTags - 1, mid_idx;
   while ( lower_idx < upper_idx )
      {
      mid_idx = lower_idx + ( upper_idx - lower_idx ) / 2;
      if ( strcmp ( TagsRecognized[ mid_idx ].Tag, inputTag ) < 0 )
         lower_idx = mid_idx + 1;
      else
         upper_idx = mid_idx;
      }

   if ( strcmp ( TagsRecognized[ lower_idx ].Tag, inputTag ) == 0 )
      return TagsRecognized[ lower_idx ].Action;
   else
      return DesiredAction::OrdinaryText;
   }